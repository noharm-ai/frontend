...
test("fast search by prescription number", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(page.getByText("Prescrição 199")).toBeVisible({ timeout: 30000 });
  const page1Promise = page.waitForEvent("popup");
  await page.getByText("Prescrição 199").click();
  const page1 = await page1Promise;
  await expect(page1.getByRole("button", { name: "199" })).toBeVisible({ timeout: 30000 });
});
...
test("fast search by admission number", async ({ page }) => {
  ...
  const page1 = await page1Promise;
  await expect(page1.getByRole("button", { name: "9999" })).toBeVisible({ timeout: 30000 });
});
