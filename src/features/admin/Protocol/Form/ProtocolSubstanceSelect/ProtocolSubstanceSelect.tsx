...
import { ProtocolSubstanceModal } from "./ProtocolSubstanceModal";

interface ProtocolSubstanceSelectProps {
  value?: Array<string | number>;
  onChange: (ids: Array<string | number>) => void;
}

export function ProtocolSubstanceSelect({
  value,
  onChange,
}: ProtocolSubstanceSelectProps) {
  const { t } = useTranslation();
  const containerId = `protocol-substance-select-${useId().replace(/:/g, "")}`;
  const [labelMap, setLabelMap] = useState<Record<string, string>>({});
  const [options, setOptions] = useState<SubstanceOption[]>([]);
  ...
  useEffect(() => {
    ...
    setResolving(true);
    api.substance
      .resolveSubstances(missing)
      .then((response) => {
        const items: SubstanceOption[] = (response.data?.data ?? []).map(
          normalizeSubstance,
        );
        ...
        setLabelMap((prev) => {
          const next = { ...prev };
          items.forEach((item) => {
            next[String(item.id)] = formatSubstanceLabel(item);
          });
          // Ids with no match (e.g. a removed substance) fall back to the bare
          // id so the effect does not keep re-requesting them.
          missing.forEach((id) => {
  ...
  const fetchData = (term: string) => {
    ...
    api.substance
      .findSubstances(term)
      .then((response) => {
        setOptions((response.data?.data ?? []).map(normalizeSubstance));
      })
      .catch((err) => {
        notification.error({
          message: getErrorMessageFromException(err.response?.data, t),
  ...
  const handleChange = (selected: any) => {
    ...
    // Keep the label of each freshly-picked substance so its tag survives the
