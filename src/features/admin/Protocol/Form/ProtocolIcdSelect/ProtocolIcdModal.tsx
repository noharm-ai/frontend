...
import { IcdOption, formatIcdLabel, normalizeIcd } from "./helpers";

interface ProtocolIcdModalProps {
  open: boolean;
  onClose: () => void;
...
export function ProtocolIcdModal({
  open,
  onClose,
  selectedIds,
  labelMap,
  onConfirm,
}: ProtocolIcdModalProps) {
  const { t } = useTranslation();
  const [term, setTerm] = useState("");
  const [rows, setRows] = useState<IcdOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [selectedKeys, setSelectedKeys] = useState<string[]>(() =>
    (selectedIds ?? []).map(String),
  );
  const [selectedItemsById, setSelectedItemsById] = useState<
    Record<string, IcdOption>
  >({});
  ...
  const fetchData = (value: string) => {
    ...
    api.lists
      .findIcds(value)
      .then((response) => {
        setRows((response.data?.data ?? []).map(normalizeIcd));
      })
      .catch((err) => {
        notification.error({
  ...
  const rowSelection = {
    ...
    onChange: (keys: React.Key[], selectedRows: IcdOption[]) => {
      ...
      setSelectedItemsById((prev) => {
        const next: Record<string, IcdOption> = {};
  ...
  const toggleRow = (record: IcdOption) => {
  ...
  const confirm = () => {
    ...
    Object.values(selectedItemsById).forEach((item) => {
      additions[String(item.id)] = formatIcdLabel(item);
    });
  ...
  const selectedLabels = selectedKeys.map((key) =>
    selectedItemsById[key]
      ? formatIcdLabel(selectedItemsById[key])
      : (labelMap[key] ?? key),
  );
  ...
  return (
    ...
    >
      <header>
        <h2 className="modal-title">Selecionar CIDs</h2>
      </header>
      ...
      <div style={{ marginBottom: "1rem" }}>
        <Input.Search
          allowClear
          value={term}
          placeholder="Digite ao menos 2 caracteres para pesquisar por descrição ou id_cid"
          loading={loading}
      ...
      <div style={{ marginTop: "1rem" }}>
        <div style={{ marginBottom: "0.5rem", fontWeight: 500 }}>
          CIDs selecionados ({selectedKeys.length})
        </div>
        <div
          style={{
            maxHeight: "140px",
        ...
        >
          {selectedKeys.length === 0 ? (
            <Empty
              ...
              description="Nenhum CID selecionado"
