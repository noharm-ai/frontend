...
import { ProtocolIcdModal } from "./ProtocolIcdModal";

interface ProtocolIcdSelectProps {
  value?: Array<string | number>;
  onChange: (ids: Array<string | number>) => void;
}

export function ProtocolIcdSelect({ value, onChange }: ProtocolIcdSelectProps) {
  const { t } = useTranslation();
  const containerId = `protocol-icd-select-${useId().replace(/:/g, "")}`;
  const [labelMap, setLabelMap] = useState<Record<string, string>>({});
  const [options, setOptions] = useState<IcdOption[]>([]);
  ...
  useEffect(() => {
    ...
    api.lists
      .resolveIcds(missing)
      .then((response) => {
        const items: IcdOption[] = (response.data?.data ?? []).map(
          normalizeIcd,
        );
        ...
        setLabelMap((prev) => {
          const next = { ...prev };
          items.forEach((item) => {
            next[String(item.id)] = formatIcdLabel(item);
          });
          // Ids with no match (e.g. a removed icd) fall back to the bare id so
  ...
  const fetchData = (term: string) => {
    ...
    api.lists
      .findIcds(term)
      .then((response) => {
        setOptions((response.data?.data ?? []).map(normalizeIcd));
      })
      .catch((err) => {
        notification.error({
          message: getErrorMessageFromException(err.response?.data, t),
  ...
  const handleChange = (selected: any) => {
    ...
    // Keep the label of each freshly-picked icd so its tag survives the
