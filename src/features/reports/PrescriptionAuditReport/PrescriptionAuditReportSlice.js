...
export const fetchReportData = createAsyncThunk(
  "reports-prescription-audit/fetch-data",
  async (params, thunkAPI) => {
    try {
      const response = await api.getReport(
        ReportEnum.PRESCRIPTION_AUDIT,
        params,
      );
      ...
      console.error(err);
      clearReportDatasource(ReportEnum.PRESCRIPTION_AUDIT);
      return thunkAPI.rejectWithValue(err.response.data);
    }
  },
);

const prescriptionAuditReportSlice = createSlice({
  name: "prescriptionAuditReport",
  initialState,
  reducers: {
    reset() {
      return initialState;
...
export const {
  ...
  setHistoryModal,
  setActiveReport,
} = prescriptionAuditReportSlice.actions;

export default prescriptionAuditReportSlice.reducer;
