...
import { getUniqBy, getLongDateRangePresets } from "utils/report";
...
export function MainFilters() {
  ...
  const handleSegmentChange = (val: string[]) => {
    setFieldValue({ segment: val, department: [] });
  };
  ...
  // the intervention dataset stores department names, so the filter value is
  // the department label (not its id)
    ...
    <>
      ...
      <Col md={6}>
        ...
        <SelectCustom
          ...
          onChange={(val: any) => handleChange("department", val)}
          showSearch
          mode="multiple"
          maxTagCount="responsive"
