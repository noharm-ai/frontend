...
export default function Filter({ idPrescriptionDrug }: FilterProps) {
  const dispatch = useDispatch();
  const isFetching =
    useSelector((state: any) => state.reportsArea.checkedIndex.status) ===
    "loading";
      ...
      <Spin spinning={isFetching}>
        {!isFetching && (
          <AdvancedFilter
            ...
            onChangeValues={() => {}}
            loading={isFetching}
            skipFilterList={["dateRange", "createdByList"]}
            memoryType=""
          />
        )}
      </Spin>
