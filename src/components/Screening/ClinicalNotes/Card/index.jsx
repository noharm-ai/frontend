  ...
  return (
    <PrescriptionCard style={{ minHeight: '113px' }}>
      ...
      <div className="footer">
        <div className="stats">
          {ClinicalNotesIndicator.listByCategory('priority', t).map(indicator => (
            <React.Fragment key={indicator.key}>
              {stats[indicator.key] > 0 && (
                <div>
                  <Tooltip title={indicator.label}>
                    <Tag className={indicator.key}>{stats[indicator.key]}</Tag>
                  </Tooltip>
                </div>
              )}
            </React.Fragment>
        ...
        <div className="action">
          <Button type="link gtm-btn-notes-all" onClick={() => setClinicalNotesVisibility(true)}>
            Visualizar
          </Button>
        </div>
      </div>
